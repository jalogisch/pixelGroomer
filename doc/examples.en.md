...
The `examples/holiday-import.sh` script imports all photos from an SD card in trip mode. No event or location — only author, copyright, and credit from the card's `.import.yaml` or .env. One folder per day by EXIF date; RAW and JPG go into `raw/` and `jpg/` by default. Ideal for post-holiday import.
...
The `examples/adventure-camp-workflow.sh` script runs import (event "Adventure Camp", location "Stadtoldendorf"; default layout: RAW in `raw/`, JPG in `jpg/` per date) and then develops RAWs with RawTherapee using a Kodak-style preset. Ideal for a two-day weekend event.
