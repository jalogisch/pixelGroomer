...
| `--split-by-type` | | RAW in `raw/`, JPG in `jpg/` with paired names (default) |
| `--no-split-by-type` | | Flat layout: all files in the date folder |
...
# Flat layout (disable default raw/ + jpg/ split)
